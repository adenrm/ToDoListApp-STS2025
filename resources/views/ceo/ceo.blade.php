...
<div class="container" style="background-color: white;height: 100vh">
    <span>Selamat datang di ceo page!</span><span style="position:relative;left:146vh"><a href="{{route('login')}}">Logout</a></span>
    <style>
        .table {
            width: 190vh;
            height: 10px
    ...
    <table class="table" border="1px">
        <tr>
            <td>
                <a href="{{route('ceo.create')}}">Buat</a>
            </td>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            ...
            <td>
                ...
                <span>
                    <form action="{{route('ceo.edit')}}" method="POST">
                        @csrf
                        <input type="hidden" name="id" value="{{$v->id}}">
    ...
    <script>
        function confirmDelete(){
            ...
            if ($bool){
                document.getElementById('deleteButton').action = "{{route('ceo.destroy')}}"
            } else {
                document.getElementById('deleteButton').method = "GET"
                document.getElementById('deleteButton').action = "{{route('ceo')}}"
            }
        }
    </script>
</div>
